...
if (isset($_POST['open_election'])) {
  $sql = "UPDATE positions SET is_open = 1";
  $conn->query($sql);
  $_SESSION['success'] = "Election opened successfully.";
}

// Check if the admin wants to close/end the election
if (isset($_POST['close_election'])) {
  $sql = "UPDATE positions SET is_open = 0";
  $conn->query($sql);
  $_SESSION['success'] = "Election closed successfully.";
}

// Get the current election status
$sql = "SELECT is_open FROM positions";
$result = $conn->query($sql);
$row = $result->fetch_assoc();
$isElectionOpen = $row['is_open'];

// Check if the user is trying to vote when the election is closed
if (isset($_POST['vote']) && !$isElectionOpen) {
  $_SESSION['error'] = "Voting is closed. Your vote is not valid.";
}
?>
...
<div class="wrapper">
  ...
  <div class="col-xs-12">
    <div class="text-center">
      <form method="post">
      <?php if (!$isElectionOpen) { ?>
          <form method="post">
            <button type="submit" name="open_election" class="btn btn-success">Start Election</button>
          </form>
        <?php } ?>
        <?php if ($isElectionOpen) { ?>
          <form method="post">
            <button type="submit" name="close_election" class="btn btn-danger">End Election</button>
          </form>
        <?php } ?>
</form>
    </div>
...
<?php
        ...
        <?php if ($isElectionOpen) { ?>
          <div class="alert alert-info">
            Election is currently <strong>open</strong>.
          </div>
        <?php } else { ?>
          <div class="alert alert-warning">
            Election is currently <strong>closed</strong>.
          </div>
        <?php } ?>

        <!-- Form to open/start the election -->
...
<!-- ./wrapper -->
<!-- additional scripts and closing tags -->
...
<script>
    $(function() {
        ...
        $("#current-time").text("Current Time: " + currentTimeString);
    });
</script> 
